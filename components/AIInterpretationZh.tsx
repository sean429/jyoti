...
export default function AIInterpretationZh({ chart, birthInfo, theme, premiumToken }: Props) {
  ...
  const cacheKey = `jyoti_interp_zh|${birthInfo.name}|${birthInfo.date}|${birthInfo.time}|${birthInfo.place}|${birthInfo.gender ?? ''}|${themeKey}`;
              ...
              <>
                ...
                {rest && (
                  <div className='relative mt-3 no-print'>
                    <p className='text-center text-[11px] font-cinzel mb-3' style={{ color: 'var(--gold-dim)', letterSpacing: '0.1em' }}>─── 免费到此 ───</p>
                    ...
                    <div className='absolute inset-0 flex flex-col items-center justify-end text-center px-5 pb-3'
                      ...
                      <p className='text-xs mb-4' style={{ color: 'var(--text-muted)', maxWidth: '400px', lineHeight: 1.6 }}>
                        今年的走势与<b style={{ color: '#e9d5ff' }}>关键时机</b>，以及塔拉的<b style={{ color: '#e9d5ff' }}>具体建议</b>，都在高级深度解读中完整揭晓。
                      </p>
                      <button
