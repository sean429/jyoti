      ...
      {!generated && !loading && (
        <div className="text-center py-6">
          <div className="mb-4">
            ...
            <p className="text-sm mb-1" style={{ color: 'var(--text-muted)' }}>Gemini AI ?? ??? ?? ??</p>
            <p className="text-xs" style={{ color: 'rgba(156,163,175,0.6)' }}>
              ??, ??, ??, ????, ?? ??? ??? ???????
            </p>
          </div>
          <button className="btn-gold" onClick={generate}>? AI ?? ?? ?? ?</button>
        </div>
      )}
      {loading && (
        <div className="text-center py-8">
          ...
          <p className="font-cinzel text-sm" style={{ color: 'var(--gold)' }}>??? ??? ???? ?...</p>
          <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>{birthInfo.name}?? ?? ??? ?? ????</p>
        </div>
      )}
      {error && (
      ...
      {interpretation && (
        <div>
          <div className="mb-4 pb-3" style={{ borderBottom: '1px solid rgba(201,168,76,0.2)' }}>
            <p className="text-xs font-cinzel" style={{ color: 'var(--gold-dim)' }}>AI ?? ?? ??</p>
          ...
          <div className="mt-6 pt-4 flex justify-center" style={{ borderTop: '1px solid rgba(201,168,76,0.1)' }}>
            <button className="text-xs font-cinzel px-4 py-2 rounded-lg hover:opacity-80"
              ...
              onClick={generate} disabled={loading}>? ?? ????</button>
          </div>
        </div>
      )}
    </div>
